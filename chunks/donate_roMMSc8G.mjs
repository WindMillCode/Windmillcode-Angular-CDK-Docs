import { I as createVNode, t as Fragment, af as __astro_tag_component__ } from './astro/server_DEq4NUVT.mjs';

const frontmatter = {
  "title": "Donate",
  "slug": "19.2.13000/donate"
};
function getHeadings() {
  return [];
}
function _createMdxContent(props) {
  const _components = {
    defs: "defs",
    span: "span",
    title: "title",
    ...props.components
  };
  return createVNode(Fragment, {
    children: [createVNode("a", {
      href: "https://github.com/sponsors/WindMillCode",
      target: "_blank",
      class: "badge github-sponsors",
      children: [createVNode("img", {
        src: "https://img.shields.io/badge/sponsor-30363D?style=for-the-badge&logo=GitHub-Sponsors&logoColor=EA4AAA",
        alt: "Github Sponsors"
      }), createVNode(_components.span, {
        "set:html": " GitHub Sponsors"
      })]
    }), "\n", createVNode("a", {
      href: "https://patreon.com/WindMillCode",
      target: "_blank",
      class: "badge patreon",
      children: [createVNode("img", {
        src: "https://img.shields.io/badge/Patreon-FF424D?style=for-the-badge&logo=patreon&logoColor=white",
        alt: "Patreon"
      }), createVNode(_components.span, {
        "set:html": "Patreon"
      })]
    }), "\n", createVNode("a", {
      href: "https://www.paypal.com/donate/?hosted_button_id=VBQJTUD9QDJW8",
      target: "_blank",
      class: "badge paypal",
      children: [createVNode("img", {
        src: "https://img.shields.io/badge/PayPal-00457C?style=for-the-badge&logo=paypal&logoColor=white",
        alt: "PayPal"
      }), createVNode(_components.span, {
        "set:html": "PayPal"
      })]
    }), "\n", createVNode("a", {
      href: "https://www.gofundme.com/f/strengthen-our-business-to-take-on-bigger-initiati?utm_campaign=p_lico+share-sheet-first-launch&utm_medium=copy_link&utm_source=customer",
      target: "_blank",
      class: "badge gofundme",
      children: [createVNode("svg", {
        class: "hrt-disp-block hrt-gfm-logo",
        fill: "none",
        height: "36",
        viewBox: "0 0 105 36",
        width: "105",
        xmlns: "http://www.w3.org/2000/svg",
        "xmlns:xlink": "http://www.w3.org/1999/xlink",
        children: createVNode("path", {
          "clip-rule": "evenodd",
          d: "m4.8556 19.4183c1.28303 0 2.73375.5394 3.50113 1.6066h.09587v-1.3308h3.4771v10.9465c0 3.6329-2.99735 5.3594-5.97097 5.3594-2.88923 0-5.70701-1.6425-5.731072-4.7238h3.453002c.08403 1.3785 1.07907 2.0141 2.23031 2.0141 1.15088 0 2.32582-.7074 2.32582-2.458v-1.6783h-.09588c-.77922.935-2.08631 1.4385-3.28531 1.4385-2.91329 0-4.8556-2.7815-4.8556-5.623s1.94231-5.5512 4.8556-5.5512zm34.4819.264v6.1986c0 .9232.7193 1.6665 1.6184 1.6665.8992 0 1.6188-.7433 1.6188-1.6665v-6.1986h3.6925v6.1386c0 3.1413-2.1581 5.0596-5.3113 5.0596-3.0932 0-5.3113-1.8465-5.3113-5.0596v-6.1386zm59.8271-.288c3.2134 0 5.7674 2.398 5.7674 5.7551 0 .3128-.029.5774-.053.7494l-.007.0539h-7.7093c.1318 1.4026 1.0069 2.194 2.0741 2.194.5951 0 1.2012-.2401 1.7242-.8148l.074-.0843h3.561c-.923 2.4698-3.177 3.6206-5.4314 3.6206-2.8892 0-5.7669-1.906-5.7669-5.7188 0-3.3571 2.5538-5.7551 5.7669-5.7551zm-79.6456-.0959c3.3453 0 6.0787 2.3621 6.0787 5.767 0 3.4052-2.7334 5.7669-6.0787 5.7669-3.333 0-6.0305-2.338-6.0305-5.7669 0-3.429 2.6975-5.767 6.0305-5.767zm53.1373-6.2345v17.5286h-3.477v-1.4508h-.0959c-.9473 1.0673-2.2662 1.6666-3.597 1.6666-2.9973 0-4.9878-2.7334-4.9878-5.6948s1.9905-5.6833 4.9878-5.6833c1.1509 0 2.5298.4316 3.3812 1.4629h.0958v-7.8292zm-40.3325-.3117c.6833 0 1.3907.0837 2.0023.2277v2.9854c-.2758-.0718-.4916-.1081-.7315-.1081-.995 0-1.8583.408-1.8583 1.7265v2.1104h2.254v2.9855h-2.254v7.9129h-3.6929v-7.9129h-1.6065v-2.9855h1.6065v-2.8774c0-2.7219 1.9305-4.0645 4.2804-4.0645zm22.684 6.6421c2.0382 0 3.9324 1.4388 3.9324 4.2444v6.9538h-3.6925v-6.1864c0-1.0913-.7315-1.8224-1.6547-1.8224s-1.6547.7552-1.6547 1.8224v6.1864h-3.6925v-10.8984h3.4767v1.3667h.0959c.8273-1.1268 2.1462-1.6665 3.1894-1.6665zm33.019 0c1.9541 0 3.8128 1.4629 3.8128 4.2085v6.9897h-3.6929v-6.3182c0-1.0195-.6715-1.6906-1.5107-1.6906-.8514 0-1.5229.6833-1.5229 1.6906v6.3182h-3.6925v-6.3182c0-1.0195-.6715-1.6906-1.5107-1.6906-.8514 0-1.5229.6833-1.5229 1.6906v6.3182h-3.6929v-10.8984h3.4771v1.3667h.0959c.7315-1.1031 1.9664-1.6543 3.1413-1.6543 1.139 0 2.3262.5993 3.0454 1.8823h.0959c.9114-1.3071 2.2181-1.8945 3.4771-1.8945zm-21.4732 3.2731c-1.3548 0-2.458 1.1031-2.458 2.4579 0 1.3549 1.1032 2.4698 2.458 2.4698s2.4576-1.1149 2.4576-2.4698c0-1.3548-1.1028-2.4579-2.4576-2.4579zm-47.0105-.0837c-1.3308 0-2.4099 1.1028-2.4099 2.4817s1.0791 2.482 2.4099 2.482 2.4098-1.1031 2.4098-2.482-1.079-2.4817-2.4098-2.4817zm-13.59622.0837c-1.29488 0-2.34988 1.0431-2.34988 2.338s1.055 2.3499 2.34988 2.3499 2.33803-1.055 2.33803-2.3499-1.04315-2.338-2.33803-2.338zm93.24182-.5634c-.971 0-1.7983.6233-2.0741 1.8705h4.0884c-.228-1.2231-1.043-1.8705-2.0143-1.8705zm3.9303-6.085.681 2.0015.676-2.0015h.456v2.3673h-.307v-1.3974c0-.0482.002-.1281.004-.2399s.003-.2317.003-.3595l-.676 1.9968h-.318l-.682-1.9968v.0726c0 .0581.002.1462.005.265.003.1189.005.2059.005.2618v1.3974h-.307v-2.3673zm-.746 0v.2821h-.798v2.0852h-.324v-2.0852h-.798v-.2821zm-49.9072-5.6385c4.1515 0 7.7733 2.2566 9.7131 5.6085h-19.4258c1.9397-3.3519 5.5615-5.6085 9.7127-5.6085zm11.1571-5.37831c.7196-.71962 1.8968-.71962 2.616 0 .7197.71925.7197 1.89641 0 2.61604l-2.7881 2.78817c-.7193.7192-1.8964.7192-2.6161 0l-.0634-.0667c-.6551-.72385-.634-1.85189.0634-2.54934zm-24.8532-.06344c.7239-.65552 1.8519-.63438 2.5494.06344l2.7885 2.78817c.7193.71925.7193 1.89641 0 2.61604-.7196.7192-1.8968.7192-2.616 0l-2.7886-2.78817c-.7192-.71963-.7192-1.89679 0-2.61604zm13.6728-4.93875.0799.00171783c.9809.04222747 1.7699.85758817 1.7699 1.84842217v3.94275c0 1.01761-.8322 1.85014-1.8498 1.85014-1.0173 0-1.8498-.83253-1.8498-1.85014v-3.94275c0-1.017614.8325-1.85014 1.8498-1.85014z",
          fill: "#02a95c",
          "fill-rule": "evenodd",
          "data-darkreader-inline-fill": "",
          style: "--darkreader-inline-fill: #06884c;"
        })
      }), createVNode(_components.span, {
        "set:html": "GoFundMe"
      })]
    }), "\n", createVNode("a", {
      href: "https://ko-fi.com/windmillcode",
      target: "_blank",
      class: "badge ko-fi",
      children: [createVNode("img", {
        src: "https://img.shields.io/badge/Ko--fi-F16061?style=for-the-badge&logo=ko-fi&logoColor=white",
        alt: "Ko-Fi"
      }), createVNode(_components.span, {
        "set:html": "Ko-Fi"
      })]
    }), "\n", createVNode("a", {
      href: "https://liberapay.com/windmillcode",
      target: "_blank",
      class: "badge liberapay",
      children: [createVNode("img", {
        src: "https://img.shields.io/badge/Liberapay-F6C915?style=for-the-badge&logo=liberapay&logoColor=black",
        alt: "LiberaPay"
      }), createVNode(_components.span, {
        "set:html": "LiberaPay"
      })]
    }), "\n", createVNode("a", {
      href: "https://opencollective.com/windmill-code",
      target: "_blank",
      class: "badge opencollective",
      children: [createVNode("img", {
        src: "https://opencollective.com/static/images/opencollective-icon.svg",
        alt: "OpenCollective"
      }), createVNode(_components.span, {
        "set:html": " OpenCollective"
      })]
    }), "\n", createVNode("a", {
      href: "https://crowdfunding.lfx.linuxfoundation.org/projects/42056b04-305f-4e08-929c-66725181d6af",
      target: "_blank",
      class: "badge community-bridge",
      children: [createVNode("img", {
        src: "https://crowdfunding.lfx.linuxfoundation.org/assets/favicons/mstile-144x144.png",
        alt: "Linux Foundation"
      }), createVNode(_components.span, {
        "set:html": " Linux Foundation"
      })]
    }), "\n", createVNode("a", {
      href: "https://tidelift.com/funding/github/npm/windmillcode",
      target: "_blank",
      class: "badge tidelift",
      children: [createVNode("img", {
        src: "https://4008838.fs1.hubspotusercontent-na1.net/hubfs/4008838/website/logos/Tidelift-logo-on-light.svg",
        alt: "Tidelift"
      }), createVNode(_components.span, {
        "set:html": " Tidelift"
      })]
    }), "\n", createVNode("a", {
      href: "https://polar.sh/windmillcode",
      target: "_blank",
      class: "badge polar",
      children: [createVNode("svg", {
        width: "42",
        height: "42",
        viewBox: "0 0 29 29",
        fill: "none",
        xmlns: "http://www.w3.org/2000/svg",
        class: "text-blue-500 dark:text-white",
        "data-sentry-element": "svg",
        "data-sentry-component": "LogoIcon",
        "data-sentry-source-file": "LogoIcon.tsx",
        children: [createVNode("path", {
          "fill-rule": "evenodd",
          "clip-rule": "evenodd",
          d: "M9.07727 23.0572C13.8782 26.307 20.4046 25.0496 23.6545 20.2487C26.9043 15.4478 25.6469 8.92133 20.846 5.67149C16.0451 2.42165 9.51862 3.67905 6.26878 8.47998C3.01894 13.2809 4.27634 19.8073 9.07727 23.0572ZM10.4703 23.1428C14.862 25.3897 20.433 23.2807 22.9135 18.4322C25.394 13.5838 23.8447 7.83194 19.4531 5.58511C15.0614 3.33829 9.49042 5.4473 7.00991 10.2957C4.52939 15.1442 6.07867 20.896 10.4703 23.1428Z",
          fill: "currentColor",
          "data-sentry-element": "path",
          "data-sentry-source-file": "LogoIcon.tsx",
          "data-darkreader-inline-fill": "",
          style: "--darkreader-inline-fill: currentColor;"
        }), createVNode("path", {
          "fill-rule": "evenodd",
          "clip-rule": "evenodd",
          d: "M11.7222 24.2898C15.6865 25.58 20.35 22.1715 22.1385 16.6765C23.927 11.1815 22.1632 5.68099 18.1989 4.39071C14.2346 3.10043 9.5711 6.509 7.78261 12.004C5.99412 17.4989 7.75793 22.9995 11.7222 24.2898ZM12.9347 23.872C16.2897 24.5876 19.9174 20.9108 21.0374 15.6596C22.1574 10.4084 20.3457 5.57134 16.9907 4.85575C13.6357 4.14016 10.008 7.817 8.88797 13.0682C7.76793 18.3194 9.57971 23.1564 12.9347 23.872Z",
          fill: "currentColor",
          "data-sentry-element": "path",
          "data-sentry-source-file": "LogoIcon.tsx",
          "data-darkreader-inline-fill": "",
          style: "--darkreader-inline-fill: currentColor;"
        }), createVNode("path", {
          "fill-rule": "evenodd",
          "clip-rule": "evenodd",
          d: "M13.8537 24.7382C16.5062 25.0215 19.1534 20.5972 19.7664 14.8563C20.3794 9.1155 18.7261 4.23202 16.0736 3.94879C13.4211 3.66556 10.7739 8.08983 10.1609 13.8307C9.54788 19.5715 11.2012 24.455 13.8537 24.7382ZM15.0953 22.9906C17.015 22.9603 18.5101 19.0742 18.4349 14.3108C18.3596 9.54747 16.7424 5.71058 14.8228 5.7409C12.9032 5.77123 11.408 9.6573 11.4833 14.4207C11.5585 19.184 13.1757 23.0209 15.0953 22.9906Z",
          fill: "currentColor",
          "data-sentry-element": "path",
          "data-sentry-source-file": "LogoIcon.tsx",
          "data-darkreader-inline-fill": "",
          style: "--darkreader-inline-fill: currentColor;"
        })]
      }), createVNode(_components.span, {
        "set:html": "Polar.sh"
      })]
    }), "\n", createVNode("a", {
      href: "https://windmillcode-angular.cheddarup.com",
      target: "_blank",
      class: "badge givebutter",
      children: [createVNode("svg", {
        width: "42",
        height: "42",
        class: "CheddarUpLogoGraphicsIcon w-auto LogoIcon-graphics h-[1.4em]",
        viewBox: "0 0 67 55",
        xmlns: "http://www.w3.org/2000/svg",
        children: [createVNode(_components.title, {
          children: "CheddarUp logo icon"
        }), createVNode(_components.defs, {
          children: createVNode("linearGradient", {
            x1: "62.3%",
            y1: "97.5%",
            x2: "34.6%",
            y2: "4.8%",
            id: ":r1v:",
            children: [createVNode("stop", {
              "stop-color": "#F36D36",
              offset: "0%",
              "data-darkreader-inline-stopcolor": "",
              style: "--darkreader-inline-stopcolor: #a93b0e;"
            }), createVNode("stop", {
              "stop-color": "#F9C442",
              offset: "100%",
              "data-darkreader-inline-stopcolor": "",
              style: "--darkreader-inline-stopcolor: #7b5a08;"
            })]
          })
        }), createVNode("path", {
          d: "M10.101 19.64l14.864 8.582a7.578 7.578 0 013.87 6.691v17.182a2.877 2.877 0 01-4.313 2.5l-9.193-5.3a1.778 1.778 0 00-1.84 0l-9.176 5.3a2.883 2.883 0 01-4.312-2.5 2.8 2.8 0 011.436-2.49l17.3-10a2.883 2.883 0 10-2.883-4.992l-9.344 5.4a.477.477 0 01-.488 0 .466.466 0 01-.241-.421v-17.46a2.877 2.877 0 014.313-2.49l.007-.002zM56.885 2.55l8.655 5a2.8 2.8 0 011.436 2.49v10a2.883 2.883 0 01-5.766 0v-7.247a1.789 1.789 0 00-.914-1.587l-6.3-3.64a2.883 2.883 0 012.889-4.986v-.03zm-6.4 7.106L35.621 1.069a7.544 7.544 0 00-7.74 0L13.017 9.656a2.877 2.877 0 000 4.981l14.864 8.587a7.555 7.555 0 007.746 0l14.858-8.587a2.877 2.877 0 000-4.981zM34.63 34.895v17.18a2.877 2.877 0 004.319 2.49l14.847-8.587a7.572 7.572 0 003.865-6.691V22.102a2.875 2.875 0 00-4.313-2.49l-14.865 8.583a7.555 7.555 0 00-3.865 6.7h.012z",
          fill: "url(#:r1v:)",
          "fill-rule": "evenodd"
        })]
      }), createVNode(_components.span, {
        "set:html": "Cheddar Up"
      })]
    })]
  });
}
function MDXContent(props = {}) {
  const {wrapper: MDXLayout} = props.components || ({});
  return MDXLayout ? createVNode(MDXLayout, {
    ...props,
    children: createVNode(_createMdxContent, {
      ...props
    })
  }) : _createMdxContent(props);
}

const url = "src/content/docs/19.2.13000/donate.mdx";
const file = "C:/My_Notebook/WindMillCode/projects/angular-templates/starlight-docs/src/content/docs/19.2.13000/donate.mdx";
const Content = (props = {}) => MDXContent({
  ...props,
  components: { Fragment: Fragment, ...props.components, },
});
Content[Symbol.for('mdx-component')] = true;
Content[Symbol.for('astro.needsHeadRendering')] = !Boolean(frontmatter.layout);
Content.moduleId = "C:/My_Notebook/WindMillCode/projects/angular-templates/starlight-docs/src/content/docs/19.2.13000/donate.mdx";
__astro_tag_component__(Content, 'astro:jsx');

export { Content, Content as default, file, frontmatter, getHeadings, url };
